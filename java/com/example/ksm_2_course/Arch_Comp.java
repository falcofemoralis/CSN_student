...
public class Arch_Comp extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_arch__comp);
    ...
    {
        Pref = getSharedPreferences("RESArch", MODE_PRIVATE);

        if (Pref.getInt("LabArch_" + Integer.toString(1) + "_0", -1) == -1)
        {
            SharedPreferences.Editor ed = Pref.edit();
            for (int i = 0; i < 6; ++i) {
                ed.putInt("LabArch_" + Integer.toString(i) + "_0", 0);
                ed.putInt("LabArch_" + Integer.toString(i) + "_1", 0);
            }
            ed.commit();
        }
        else
            for (int i = 0; i < 6; ++i)
            {
                if (Pref.getInt("LabArch_" + Integer.toString(i) + "_0", -1) == 1) {
                ...
                if (Pref.getInt("LabArch_" + Integer.toString(i) + "_1", -1) == 1) {
    ...
    protected void SaveAll()
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (((ColorDrawable)buts[i][0].getBackground()).getColor() == 0xFFDFFFBF)
                ed.putInt("LabArch_" + Integer.toString(i) + "_0", 1);
            else
                ed.putInt("LabArch_" + Integer.toString(i) + "_0", 0);

            if (((ColorDrawable)buts[i][1].getBackground()).getColor() == 0xFFDFFFBF)
                ed.putInt("LabArch_" + Integer.toString(i) + "_1", 1);
            else
                ed.putInt("LabArch_" + Integer.toString(i) + "_1", 0);
        }
        ed.putInt("RESArch", complete * 100 / 12);
        ed.commit();
