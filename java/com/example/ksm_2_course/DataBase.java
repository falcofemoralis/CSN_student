...
public class DataBase extends AppCompatActivity {
    ...
    {
        Pref = getSharedPreferences("RESDB", MODE_PRIVATE);

        if (Pref.getInt("LabDB_" + Integer.toString(1) + "_0", -1) == -1)
        {
            SharedPreferences.Editor ed = Pref.edit();
            for (int i = 0; i < 6; ++i) {
                ed.putInt("LabDB_" + Integer.toString(i) + "_0", 0);
                ed.putInt("LabDB_" + Integer.toString(i) + "_1", 0);
            }
            ed.commit();
        }
        else
            for (int i = 0; i < 6; ++i)
            {
                if (Pref.getInt("LabDB_" + Integer.toString(i) + "_0", -1) == 1) {
                ...
                if (Pref.getInt("LabDB_" + Integer.toString(i) + "_1", -1) == 1) {
    ...
    protected void SaveAll()
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (((ColorDrawable)buts[i][0].getBackground()).getColor() == 0xFFDFFFBF)
                ed.putInt("LabDB_" + Integer.toString(i) + "_0", 1);
            else
                ed.putInt("LabDB_" + Integer.toString(i) + "_0", 0);

            if (((ColorDrawable)buts[i][1].getBackground()).getColor() == 0xFFDFFFBF)
                ed.putInt("LabDB_" + Integer.toString(i) + "_1", 1);
            else
                ed.putInt("LabDB_" + Integer.toString(i) + "_1", 0);
        }
        ed.putInt("RESDB", complete * 100 / 12);
        ed.commit();
    }
